...
"""
import numpy as np
from CBBA_rewardFun import Scoring_CalcScore_Original, Scoring_CalcScore_DNN, reward_function_DRL, Scoring_CalcScore_DRL,\
    Scoring_CalcScore_Maxmin_DNN
    


class CBBA():
    
    def __init__(self, obs, scorefun, Graph, **kwargs):
        # define CBBA constants
        self.CBBA_Params_N = len(obs) # number of agents
        self.CBBA_Params_M = np.size(obs[0],1) #number of tasks
        self.CBBA_Params_MAXDEPTH = np.size(obs[0],1) #maximum bundle depth (remember to set it for each scenario)
        ...
        self.env = kwargs.get('env') if 'env' in kwargs else None

        if self.DRL_model:
            self.reward_function_DRL = reward_function_DRL(self.env, self.DRL_model)
    ...
    def CBBA_BundleAdd(self,n):
        ...
        bundleFull = not len(np.argwhere(CBBA_Data['bundle']==-1))
        value = 1
    ...
    def CBBA_ComputeBids(self, CBBA_Data,n):
        ...
        if self.Scoring_CalcScore == 'Scoring_CalcScore_Original' or self.Scoring_CalcScore == 'Scoring_CalcScore_Robust':
            if self.Scoring_CalcScore == 'Scoring_CalcScore_Original' :
                ...
                robust_greedy = True
            for m in range(self.CBBA_Params_M):
                if not len(np.argwhere(CBBA_Data['path']==m)):
                    ...
                    for j in range(L[0][0]+1):
                        ...
                        score_old, score_new, minStart, maxStart = Scoring_CalcScore_Original(self.obs[n], m, 
                                                                                 taskPrev,timePrev,
                                                                                 taskNext,timeNext,
            ...
            score_old, score_new, minStart, maxStart = eval(self.Scoring_CalcScore)(self.obs[n], self.value_model, unAllocatedTask, taskPrev, timePrev, taskNext,timeNext)
            original_score = np.sum(CBBA_Data['scores'][:L[0][0]])
            score = score_new - original_score
            ...
            score, minStart, maxStart = eval('Scoring_CalcScore_DNN')(self.obs[n], self.value_model, unAllocatedTask, taskPrev, timePrev, taskNext, timeNext)
            ...
            score_old, score_new,  minStart, maxStart = eval(self.Scoring_CalcScore)(self.obs[n], self.value_model, self.value_model2, unAllocatedTask, taskPrev, timePrev, taskNext,timeNext)
            original_score = np.sum(CBBA_Data['scores'][:L[0][0]])
            score = score_new - original_score
            ...
            score_old, score_new,  minStart, maxStart = Scoring_CalcScore_DNN(self.obs[n], self.value_model, unAllocatedTask, taskPrev, timePrev, taskNext,timeNext)
            original_score = np.sum(CBBA_Data['scores'][:L[0][0]])
            score = score_new - original_score
            
            score_old, score_new,  minStart, maxStart = Scoring_CalcScore_DNN(self.obs[n], self.value_model2, unAllocatedTask, taskPrev, timePrev, taskNext,timeNext)
            original_score = np.sum(CBBA_Data['scores'][:L[0][0]])
